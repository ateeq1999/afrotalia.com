        ...
        {services.map((s, i) => (
          <div key={s.id}>
            <div className="grid grid-cols-1 gap-6 py-12 sm:py-16 md:grid-cols-[120px_1fr]">
              <p className="text-[15px] font-bold tabular-nums text-caption">{String(i + 1).padStart(2, "0")}</p>
              <div>
                <h2 className="text-[26px] font-bold tracking-tight sm:text-[30px]">{s.name}</h2>
                <p className="mt-3 max-w-[640px] text-[16px] leading-relaxed text-muted-ink">{s.summary}</p>
              </div>
            </div>
