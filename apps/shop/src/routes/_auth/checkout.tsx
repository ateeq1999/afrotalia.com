...
const inputClass = cn(
  ...
  "placeholder:text-caption focus:border-brand-green-700 disabled:cursor-not-allowed disabled:opacity-50",
);
        ...
        <li key={label} className="flex items-center gap-2">
          <span
            className={cn(
              ...
              index <= step ? "border-brand-green-700 bg-brand-green-700 text-white" : "border-hairline text-muted-ink",
            )}
          >
            {index + 1}
          </span>
          <span className={index === step ? "text-ink" : "text-muted-ink"}>{label}</span>
          {index < STEPS.length - 1 ? <span className="mx-1 h-px w-6 bg-hairline" aria-hidden /> : null}
        </li>
      ))}
    </ol>
  );
  ...
  return (
    <main className="bg-white font-sans text-ink antialiased">
      <div className="mx-auto w-full max-w-[720px] px-4 pb-16 pt-8 sm:px-5">
        ...
        <div className="mt-6 rounded-xl border border-hairline p-5 sm:p-6">
          {step === 0 ? (
            <div>
                  ...
                  {addresses.map((a) => (
                    <label
                      ...
                      className={cn(
                        ...
                        addressId === a.id ? "border-brand-green-700 bg-brand-green-700/5" : "border-hairline",
                      )}
                    >
                      <input
                        type="radio"
                      ...
                      <span>
                        ...
                        <span className="block text-muted-ink">{a.phone}</span>
                      </span>
                    </label>
                  ))}
                  <button
                  ...
                  <div className="flex gap-2 sm:col-span-2">
                    <button
                      ...
                      className={cn(
                        ...
                        savingAddress ? "bg-surface-muted text-caption" : "bg-brand-green-700 hover:bg-brand-green-700/90",
                      )}
                    >
                    ...
                    {addresses.length > 0 ? (
                      <button
                        ...
                        className="h-11 rounded-lg border border-hairline px-4 text-[14px] font-medium"
                      >
                        Cancel
                      </button>
                    ) : null}
                ...
                {initial.deliveryMethods.map((m) => (
                  <label
                    ...
                    className={cn(
                      ...
                      deliveryMethodId === m.id ? "border-brand-green-700 bg-brand-green-700/5" : "border-hairline",
                    )}
                  >
                    <span className="flex items-start gap-3">
                      ...
                      <span>
                        ...
                        <span className="block text-muted-ink">{m.description}</span>
                        <span className="block text-muted-ink">{m.etaLabel}</span>
                      </span>
                    </span>
            ...
            <div>
              ...
              <dl className="mt-4 space-y-2 text-[13px]">
                <div className="flex justify-between">
                  <dt className="text-muted-ink">Items</dt>
                  <dd className="font-medium tabular-nums">{formatTZS(initial.itemsTotal)}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-muted-ink">Delivery ({selectedDelivery?.name})</dt>
          ...
          <div className="mt-5 flex items-center justify-between gap-3">
            <button
              ...
              className={cn(
                "h-11 rounded-lg border border-hairline px-5 text-[14px] font-medium transition-colors",
                step === 0 ? "cursor-not-allowed opacity-40" : "hover:border-ink/20",
              )}
            >
              Back
            </button>
            {step < 3 ? (
              ...
              <button
                ...
                className={cn(
                  "h-11 rounded-lg px-6 text-[14px] font-bold text-white transition-colors",
                  submitting ? "cursor-not-allowed bg-surface-muted text-caption" : "bg-brand-green-700 hover:bg-brand-green-700/90",
                )}
              >
